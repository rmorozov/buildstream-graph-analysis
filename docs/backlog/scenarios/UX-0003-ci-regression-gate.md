...
2. A distinct exit code (not reusing 1/2/3, which already mean "general error"/"ingestion failure"/"analysis failure") so CI logs can distinguish "the build got worse" from "bga itself broke" - document the new code in `docs/guides/cli.md`'s Exit Codes table alongside the existing four.
...
3. **Distinct exit code `4`** (`bga/cli.py`'s `EXIT_CODE_REGRESSION`) - separate from `1`/`2`/`3`, which all mean "`bga` itself failed to run"; `4` means the opposite, `bga` ran successfully and is reporting a real regression in the *analyzed build*. Documented in `docs/guides/cli.md`'s Exit Codes table.
...
6. **Worked GitHub Actions example** added to `docs/guides/cli.md`'s `bga compare` section, plus the full flag/exit-code documentation.
...
4. `docs/guides/cli.md` documents the new flag and exit code with a real, verified example.
5. Full suite green.
