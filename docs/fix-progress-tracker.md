...
| P1-20 | Blame-chain walk never classifies a gap as RESOURCE_WAIT/SCHEDULER_WAIT — always defaults to DEPENDENCY_WAIT, so `P1-01`/`P1-02`'s correct classifiers never reach final output | P1-01 (done), P1-02 (done) | 🔴 NEW, found while finishing `P1-01`, root cause precisely identified | [P1-20](tasks/P1-20-gap-classification-into-resource-scheduler-wait.md) |
...
`P1-01`, `P1-02`, `P1-03`, `P1-04`, `P1-18`, `P1-19`, `P2-01`, `P2-02`, `P2-05`, and `P3-02` are now done (see Change Log). The whole "flattened timeline / attribution identity" family (`P1-03`, `P1-04`, `P1-19`) is closed — attribution identity holds exactly on every fixture tested. `P1-01`/`P1-02`'s classifiers are also now correct and unit-tested, but finishing `P1-01` surfaced that their output never actually reaches the final report (`P1-20`). Next up:

1. `P1-20` (wire `P1-01`/`P1-02`'s now-correct classifiers into the flattened timeline - currently `resource_wait_us`/`scheduler_wait_us` are always `0` in final output regardless of what the trace shows, per spec Part 7's explicit "the interval is classified according to what happened during that gap")
2. `P2-03` (logging infrastructure — cheap, high diagnostic value for every session after)
