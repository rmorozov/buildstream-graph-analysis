...
Mechanical and bounded shapes are `implementer` tracks on `sonnet` in worktrees; a judgement shape is the session's own — the *judgement*, not the work: the rows that say the session took it in the brief ran on `sonnet` for a 190k median against bounded's 252k (10 of 21 runs, all merged; `dev_process_bands.py --runs`).
