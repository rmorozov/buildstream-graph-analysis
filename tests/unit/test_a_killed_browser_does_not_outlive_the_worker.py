...
class TestAKilledBrowserDoesNotOutliveTheWorker:
    ...
    def test_a_second_entry_sweeps_the_first(self, isolated_tmp):
        before = len(_profiles(isolated_tmp))
        profile = _launch_and_kill(isolated_tmp)

        # The positive control: without it, "gone afterwards" would be
        # satisfied by a launcher that never leaked anything.
        ...
        assert left, (
            "no process names the leaked profile - reparenting did not "
            "happen the way this guard assumes, so it would prove nothing")
        ...
        try:
            with Browser(CHROME):
                ...
                assert after == before + 1, (
                    f"expected only the second entry's own root "
                    f"(before={before}+1), got {after}")
        finally:
            # This entry becomes `_SHARED` and would otherwise outlive
            # the `with`, still running when `isolated_tmp` removes its
            # root at teardown - closed here rather than left for this
