...
def write_area_pages():
    """`UX-688`: one generated page per area, and the paths written.

    A view, not a move: the task files stay flat and their 714 guarded
    links stay valid. Each page is regenerated whole from the headers,
    so it cannot drift from them - editing a page by hand is undone by
    the next `--check --write`, which is the point.

    `UX-689`: a hand-written `docs/design/areas/<area>.md`, if one
    exists, gets one derived `Mechanism:` line - so the generated page
    points at the prose rather than a later round pasting it in and
    losing the view.
    """
    pages = area_pages()
    if not pages:
        return []
    AREA_PAGES.mkdir(parents=True, exist_ok=True)
    wrote, keep = [], set()
    for area, ids in pages.items():
        ...
        body = (f"# {area}\n\n"
                f"Generated by `dev_close_task.py --check --write` "
                f"(`UX-688`) from each task's `**Area:**` header. "
                f"{len(listed)} row(s); the module tree is the fixing "
                f"guide's §6.\n\n{mechanism}| Task | Topic |\n|---|---|\n{rows}\n")
        path = AREA_PAGES / name
        if not path.exists() or path.read_text(encoding="utf-8") != body:
            path.write_text(body, encoding="utf-8")
            wrote.append(path)
    for stale in AREA_PAGES.glob("*.md"):
