...
| `dev_close_task.py UX-NNN --move --note-file <path>` then `--counts` | the row move and both markers, then the derived counts printed, never committed (UX-996); `--note-file` keeps the note off the command line (UX-768) |
...
Mechanical and bounded shapes are `implementer` tracks on `sonnet` in worktrees; a judgement shape the `architect` has not shaped is the session's own — the *judgement*, not the work: those rows ran for a 318k median against bounded's 228k (73 of 163 runs; `dev_process_bands.py --runs`). The architect is pulled, never a gate.
