...
How Planes 1 and 2 are joined on the element UID by `bga/correlate.py`, why a merged capture would buy nothing, and the four properties that make the join trustworthy, is in [`docs/design/areas/bga.md`](areas/bga.md) (`UX-816`).

## The ingestion path now measures itself (`UX-105`–`UX-110`)

How Plane 1 and Plane 2's own ingestion mechanisms measure their
own blind spots — Plane 2's static-linkage census and optional
ptrace spine, the one joined process list, and Plane 1's own
timestamp-resolution check against BuildStream's elapsed prefix —
is in [`docs/design/areas/tools.md`](areas/tools.md) (`UX-815`).

## What the 2026-08-16 audit round changed structurally

The three ways `UX-27`..`UX-40` changed the shape of what the tool asserts — two efficiency numbers instead of one, a single capacity verdict everything conditions on, and two independent CI gates instead of one threshold — is in [`docs/design/areas/bga.md`](areas/bga.md) (`UX-816`).

## What the real-capture rounds (7–10) changed structurally

What auditing against a real `freedesktop-sdk` capture changed from round 7 — ranking by realizable saving rather than share of path, one capture answering several questions, findings published as data, and the caches-off gap `UX-86`/`UX-170` closed — is in [`docs/design/areas/bga.md`](areas/bga.md) (`UX-816`).

## Core invariants still load-bearing (Plane 1)

The spec's invariants still load-bearing on Plane 1 — I4, I8, I9, I11, I12 — and which guard exercises each, is in [`docs/design/areas/bga.md`](areas/bga.md) (`UX-816`).
...
"build_class_comparison", "baseline_band_sources"]`), in `bga/buildclass.py`, which owns the
...
the 2026-08-16 audit round changed structurally", "What the
real-capture rounds (7–10) changed structurally", "Core invariants
still load-bearing (Plane 1)" and "Real extensions beyond the original
spec"; 17, 72, 20, 10 and 86 lines including each heading) moved into a
new hand-written `docs/design/areas/bga.md`, following `UX-815`'s
tools-area move. The move is re-grounded in the 21 test files naming
`architecture.md`: blanking each chapter's body in place and rerunning
all of them before writing the replacement found the fifth chapter's
history table read whole by `test_the_architecture_table_is_read_at_all`
and `test_the_table_status_matches_the_task_files`, so that chapter's
prose and table stay in architecture.md unchanged and the new page
carries only a pointer back to it; the other four chapters had no line
read by any guard and moved wholly, leaving a heading and a
one-paragraph pointer each. `python3 -m pytest $(grep -ln
"architecture.md" tests/unit/*.py) -q` stayed at 437 passed before and
after, and a diff of the removed prose against the new page's body was
...
and `bga/viewer/` still **22 modules** (`ls
bga/viewer/*.js | wc -l`). The item published no id and moved no key.

Updated 2026-09-12 (after `UX-815`), covering one change to this
document — "The ingestion path now measures itself" chapter's
mechanism prose moved into `docs/design/areas/tools.md` as its second
section, after `UX-810`'s Plane 3 chapter, leaving the heading and a
...
against the new section's body was empty. The two contract tables
above are unchanged since the last entry: **25 emitted ids, 10 of them
