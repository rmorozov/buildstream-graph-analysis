...
Every step of that is supported except the arrows. `docs/optimization-walkthrough-06.md` records the consequence: the walkthrough's findings came out of two JSON files and an ad-hoc script, and its closing note still said the macro and micro halves "remain two tools and two captures".
