...
Part 35 (Determinism Contract) / I11 - "the spec explicitly requires ≥100 canonical-output comparisons" (already the harness's own stated design point, `bga/validation/determinism.py:5-10`, `docs/backlog/tasks/P1-12-determinism-harness.md`).
...
3. The new check is added to the normal test suite (gated behind whatever "slow" marker convention this repo already uses, matching `docs/backlog/tasks/P3-07-montecarlo-and-determinism-tests.md`'s "gate the golden/determinism/Monte-Carlo tests behind a slower marker" precedent) so it runs in CI without materially slowing down the default `make test`/`make lint` loop.
4. Full suite green.
