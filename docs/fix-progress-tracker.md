...
**Overall Status:** P0 verified fixed. P1 essentially complete — every item done except `P1-14` (blocked on a product decision) and `P1-15` (a pure architecture refactor, deliberately deferred to its own session now that its dependencies are satisfied). See `P1-21` for newly-found performance follow-ups. P2/P3 partially done — see tables below.
...
| P1-15 | Missing `bga/floors/`, `bga/report/`, `bga/validation/` packages (architecture) | P1-06..P1-13 (all now done) | 🔴 ready to start — deliberately deferred this round, see note below | [P1-15](tasks/P1-15-package-architecture-refactor.md) |
...
| P1-21 | Additional O(N²)-ish hotspots found while verifying `P1-16` (not in its original scope): `compute_reachability`'s full-set materialization, `compute_dominators`'s naive dataflow, `_estimate_ready_count`'s O(N²) `any()` loop | — | 🔴 NEW, found 2026-08-13 while verifying `P1-16`, root causes precisely identified | [P1-21](tasks/P1-21-additional-performance-hotspots.md) |
...
Every P1 item except `P1-14` (blocked on a product decision) and `P1-15` (deliberately deferred, see below) is now done, along with `P2-01`, `P2-02`, `P2-03`, `P2-05`, and `P3-02` (see Change Log). All hard-invariant families (attribution identity I4, capacity lower bound I1/I2, confidence/reconciliation gates I11) hold exactly on every fixture tested. Next up:

1. `P2-04` (retry/rebuild detection - cheap, unblocks real `WASTED_RETRY`/`WASTED_REBUILD` utilization values)
2. `P1-21` (the newly-found performance hotspots `P1-16`'s verification surfaced - `compute_reachability`, `compute_dominators`, `_estimate_ready_count`)
3. `P1-15` (package architecture refactor) - now that `P1-06`..`P1-13` are all done and verified, this is unblocked. Deliberately not attempted in the same round as the ten correctness fixes above: it's a pure, zero-behavior-change refactor touching the same files those fixes just landed in, and its own acceptance test demands incremental, byte-identical-output-diffed verification after *each* extraction step - a careful, dedicated session, not a tail-end addition to an already-large batch.
