...
`P1-01`, `P1-02`, `P1-03`, `P1-04`, `P1-18`, `P1-19`, `P1-20`, `P2-01`, `P2-02`, `P2-03`, `P2-05`, and `P3-02` are now done (see Change Log). The whole "flattened timeline / attribution identity" family (`P1-03`, `P1-04`, `P1-19`, `P1-20`) is closed — attribution identity holds exactly on every fixture tested, and `resource_wait_us`/`scheduler_wait_us` are now real, non-zero values wherever the trace evidence supports them. Logging (`P2-03`) is now wired end-to-end, so every subsequent session can pass `--verbose`/`--log-file` to actually see what a run did. Next up:

1. `P1-05` (raise a violation when the timeline undercounts - now that `P1-04`/`P1-20` have closed the known undercounting gaps, this becomes a genuine safety net rather than papering over a live bug)
2. `P2-04` (retry/rebuild detection - cheap, unblocks real `WASTED_RETRY`/`WASTED_REBUILD` utilization values)
