...
2. **Efficiency Score formula wrong in cli.md.** `docs/guides/cli.md:77` says
...
3. **Exit code 4 is documented too narrowly.** `docs/guides/cli.md:341` says
...
4. **`docs/guides/cli.md:82`** ("Key Findings … presentation-only … JSON
   unaffected") predates UX-75 and contradicts `docs/guides/cli.md:96`.
...
9. **`--cold` with no history prints nothing at all** in text
   ...
   unavailable" (`docs/guides/cli.md:151`).
10. Smaller: compare verdict values omit `not comparable …`
    (`bga/compare.py:285`); "verbatim" JSON snippets omit the `rows`
    array; `docs/design/architecture.md:280` lists UX-60 as Done vs the
    backlog's 🟡; `docs/design/architecture.md:217` "all 22 scenario files"
    (there are 76); `docs/spec/ingestion-pipeline.md` fact 5 contradicted by
...
| 2 | Efficiency Score = `LB / total duration` | `LB / horizon`; on the golden fixture 1.00 vs 0.875 | `docs/guides/cli.md` **and** the code comment at `bga/analyzer.py` |
...
| 5 | no published `findings[].id` list | 15 in `bga/findings.py`, 9 correlate rows + 1 restructuring | two tables in `docs/guides/cli.md`; every id checked present |
