...
**Status:** landed — argued here; the twelve rows `UX-841` to `UX-852` landed in round 118 as the mode's three stages, on the spike `UX-679` from round 100; the evaluation row (`examples/10-jobserver`, four elements on four cores) read the mode slower at every stage - off 21.62 s, auto 22.83 s, then 32.23 s without and 41.41 s with a plan on the merged tree - so the mode ships as a capture option, off by default; round 119's `examples/11-serial-giant` (one long element under `max-jobs: 2` on four cores, `UX-857`) gave the first positive reading on a quiet box - auto IMPROVED -10.6 %, 286.53 s to 256.06 s, the element's peak 2 to 3 - and the server's own number (39 tokens against `max-jobs` 8) waits on such a box. Round 120's field capture found the pool never grew past its opening seed with builders equal to the cores and manual-kind recipes never joined; `UX-858` and `UX-859` closed both, and the scaled pair on four cores read `auto` IMPROVED -13.4 %. The first real-core reading (16 Cortex-A72, CodSpeed Graviton runner, `examples/11-serial-giant` at its committed 9800 lines/unit, staged via `UX-1009`, 3 interleaved repeats, `bst` run 36086044196): `off` averaged 139.27 s wall (139.91/139.20/138.71, spread 0.9 %), `auto` averaged 112.56 s (113.28/112.19/112.20, spread 1.0 %) - `auto` IMPROVED -19.2 %, the giant's peak concurrency 8 to 16, host CPU seconds 751 to 842 (+12 %). A second pair on the same box pinned `max-jobs: 3` (the 8-of-40 server shape on 16 cores, run 36095261434): `off` averaged 261.4 s wall, `auto` 112.3 s - `auto` IMPROVED -57.0 %, width 3 to 16, host CPU seconds 729 to 841 (+15 %), host used-memory peak 448 M to 1611 M. The compile-bound case at scale is still open: the jobserver block's own two shares (tokens-idle, tokens-starved) are unread for either pair, and the memory figure is host-level, not per-element.
