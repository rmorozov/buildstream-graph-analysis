...
## Required Fix
1. `grep -rniE "interval eclipsing|mathematically optimal|cold floor.*certified|resource blocker" bga/ docs/cli.md README.md` (adjust the pattern list to match the full avoid-list from Part 43 exactly — read it first, don't guess the terms).
2. For every match in user-facing output strings (CLI help text, report formatting strings, docstrings that get surfaced to users, README/docs/cli.md prose), replace with the preferred equivalent term from the same Part 43 table.
...
## Out of Scope
- Don't touch `docs/specification.md` itself (never edit the spec, per `docs/fixing-guide.md` §5).
