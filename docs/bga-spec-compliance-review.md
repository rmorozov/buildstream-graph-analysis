...
- **Cycle detection is entirely absent.** `docs/cli.md:126-131` documents exit code 3 for "graph cycles detected," but no code checks for cycles. Kahn's-algorithm-based depth computation silently defaults unreached (i.e. cyclic) nodes to depth 0 instead of erroring (`bga/graph/edg.py:108-112`).
