...
Mechanical and bounded shapes are `implementer` tracks on `sonnet` in worktrees; a judgement shape is the session's own — the *judgement*, not the work: three of the four judgement rows say the session took it in the brief, and those tracks ran on `sonnet` for a 144k median against bounded's 239k (4 of 17 runs, all merged; `dev_process_bands.py --runs`).
