...
## Spec Reference
`sed -n '2133,2182p' docs/specification.md` (Part 37 — CLI). The spec recommends `bga analyze|graph|floors|replay|sweep|utilisation|diagnostics RUN`, and Part 37.1 gives literal examples of `bga floors RUN --cold` / `bga floors RUN --cold --allow-partial-cold`.
...
5. `docs/cli.md` documents the new subcommands.
