...
| R2 | **The recipe author** — owns elements and their sources | the cost of *their* element: sandbox tax, achieved parallelism, what a change to their repo rebuilds | their local fix pessimising someone else's build | **Served.** The element object (`correlate/v2`), blast by resource, element history, Plane 2 lanes |
