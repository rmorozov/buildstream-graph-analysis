...
| [120](../audits/round-120.md) | in progress: a 16-core field capture read eight things off the page - the pool that never grew past its opening seed (`UX-858`), manual recipes that never joined (`UX-859`), swap unpublished (`UX-860`), the capacity figure unclamped (`UX-861`), the twin's CSS (`UX-862`), the strip's ticks (`UX-863`), maps as pairs (`UX-864`), relative opens dropped (`UX-865`); four `researcher` reads ground-truthed each before filing |
