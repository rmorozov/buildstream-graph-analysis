...
#: unused below - kept importable as `tracer.<name>` so existing tests
#: (`test_a_per_element_width_series_keeps_its_clock.py`,
#: `test_the_token_ledger_has_two_shares.py`) keep resolving them off
...
# UX-901: `PoolController` samples `/proc/stat` through this same reader
# too - bound here rather than imported, since `tools/jobserver/` never
# reaches back into this module.
bind_cpu_sampler(read_cpu_sample)
...
# UX-901: `Broker`'s own live refresh (`_maybe_refresh_pid_to_element`)
# reads through this same reader - bound here rather than imported, for
# the same reason as `cpu_sampler` above.
bind_pid_to_element_reader(read_pid_to_element)
