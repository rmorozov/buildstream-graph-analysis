...
6. **Time-of-extraction consistency check**: if `project_dir` is a git repository, warns (does not fail) when the working tree is dirty (`git status --porcelain`) - see `docs/ingestion-pipeline.md`'s "time-of-extraction consistency" note for the honest limitation (a clean-but-wrong-commit tree isn't detectable this way).
