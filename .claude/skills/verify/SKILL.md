...
- **Last, run the `self-review` skill on the diff** (`UX-701`). It
  ...
  skill goes to `design-review` as well if it touches the page, and
  to `review`'s routed-diff section if it does not (`UX-928`) — then
  reads `REVIEW.md`'s four passes against what you wrote. The
  session's own model reading its own diff is what that replaces.
