...
filed (`UX-924`), and one guard waived so the round could reach a green
...
filed    UX-924   adopt reads the candidate's median, not its reading
...
`main` agrees: over the **37 commits** the adopt job made to
`tests/ci_reference.json` between `6ad0d884` (2026-09-14) and
`4145a45c` (2026-09-22), this entry's window only ever held 6.47 and a
single 6.48 walking through it — the rounding of the carried values, never a reading. 543 of 566
...
remaining rows are the same shape as this one until `UX-924` lands: a
...
|---|---|---|---|---|---|---|
| self-review | sonnet | `UX-908` close + `UX-924` filing | 151k | 58 | 8.5 m | confirming the hand-append matched the `verify` skill's raw/shift rule needed tracing `flake_ledger.json`'s stored `shift` field back through `dev_tier_drift.py`'s own `ratio / shift`, since it is not the run's printed drift shift |
