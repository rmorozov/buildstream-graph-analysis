...
| [121](../audits/round-121.md) | in progress: the first day on a real project - the jobserver FIFO bound onto its own host path under a read-only root (`UX-869`), the kinds read dropping the user's own options (`UX-870`), a junctioned element's kind never found (`UX-871`), a junctioned example under the snapshot entry point in CI (`UX-872`), plus review 24's three rows (`UX-866` to `UX-868`) |
