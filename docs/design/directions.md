...
**Status:** landed — the four corrections are argued here; `UX-675`, `UX-676`, `UX-677`, `UX-681` landed first, `UX-678`, `UX-680`, `UX-682`, `UX-683`, `UX-684` in rounds 110-111, `UX-679` (the spike: a prototype, not a mode) and the price `UX-739` in round 112.
...
**Status:** landed — argued here; the twelve rows `UX-841` to `UX-852` landed in round 118 as the mode's three stages, on the spike `UX-679` from round 100; the evaluation row (`examples/10-jobserver`, four elements on four cores) read the mode slower at every stage - off 21.62 s, auto 22.83 s, then 32.23 s without and 41.41 s with a plan on the merged tree - so the mode ships as a capture option, off by default; round 119's `examples/11-serial-giant` (one long element under `max-jobs: 2` on four cores, `UX-857`) gave the first positive reading on a quiet box - auto IMPROVED -10.6 %, 286.53 s to 256.06 s, the element's peak 2 to 3 - and the server's own number (39 tokens against `max-jobs` 8) waits on such a box. Round 120's field capture found the pool never grew past its opening seed with builders equal to the cores and manual-kind recipes never joined; `UX-858` and `UX-859` closed both, and the scaled pair on four cores read `auto` IMPROVED -13.4 %. The real-core reading (`UX-905`, closed): 16 Cortex-A72, CodSpeed Graviton runner, `examples/11-serial-giant` at its committed 9800 lines/unit, staged via `UX-1009`, 3 interleaved repeats per arm, reproduced by `examples/11-serial-giant/graviton_arms.sh`'s `pairs`/`cap3` legs, driven by `.github/workflows/codspeed-probe.yml`'s matrix. Direction 20's stage 2 was running: pool mode `dynamic`, the token ledger present, no `--plan` (stage 3's per-element priority not engaged). `pairs` (bst's own `max-jobs` 8): `off` 139.3 s wall -> `auto` 112.2 s, IMPROVED -19.5 % (run 36103304381; a first run, 36086044196, read -19.2 %), host CPU 752 s -> 841 s (+12 %), host used-memory peak 887 M -> 1623 M. `cap3` (`max-jobs: 3`, the 8-of-40 server shape): `off` 261.1 s -> `auto` 112.4 s, IMPROVED -57.0 % (run 36103304381; a first run, 36095261434, read -57.0 %, agreeing within 0.5 %), host CPU 733 s -> 843 s (+15 %), host used-memory peak 446 M -> 1581 M - the numbers are a machine's, the procedure reproduces them. The jobserver block's two shares, `auto` arms only: tokens-idle 0.56, tokens-starved 0.00 on every repeat of both legs - the pool never ran dry while cores idled; inference, not measured directly: the 56 % of ticks with tokens on offer and cores idle are the build's serial phases (toolchain import, cmake configure/link, the leaves), where nothing can claim them.
