...
**Every P1 item (P1-01 through P1-22) is now done.** `P2-01`, `P2-02`, `P2-03`, `P2-05`, and `P3-02` are also done (see Change Log). All hard-invariant families (attribution identity I4, capacity lower bound I1/I2, confidence/reconciliation gates I11) hold exactly on every fixture tested, the CLI covers the full spec Part 37 command list, and the codebase now matches spec Part 39's recommended `bga/floors/`/`bga/report/`/`bga/validation/` architecture. The remaining P1 work is entirely follow-up items that weren't judged worth doing in the same round they were found - see `P1-15`'s task file note on the `bga/graph/`/`bga/attribution/` further-splitting that Part 39 also recommends but this task never scoped in. Next up:

1. `P2-04` (retry/rebuild detection - cheap, unblocks real `WASTED_RETRY`/`WASTED_REBUILD` utilization values)
