...
Mechanical and bounded shapes are `implementer` tracks on `sonnet` in worktrees; a judgement shape is the session's own — the *judgement*, not the work: the rows that say the session took it in the brief ran on `sonnet` for a 318k median against bounded's 249k (72 of 141 runs, four whose own cell disagrees; `dev_process_bands.py --runs`).
