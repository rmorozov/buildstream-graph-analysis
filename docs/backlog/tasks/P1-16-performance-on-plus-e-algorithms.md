...
Read only: `sed -n '2576,2652p' docs/spec/specification.md` (Part 41 — Performance Requirements). Quoted complexity targets: `graph construction: O(N+E)`, `critical path: O(N+E)`, `blast radius: O(N+E) with reverse traversal/memoization`. "Avoid O(N²) for routine diagnostics."
