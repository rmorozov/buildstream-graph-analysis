...
| `agent-runs.md:41` | *the twenty-four rows* | **26** — this review first said 25, from `awk '/^\| /{n++} END{print n-2}'`: the pattern needs `\| ` and never matched the `\|---\|` separator, which the `-2` subtracted regardless. `UX-734`'s track re-derived it and the sentence now reads *twenty-six* |
