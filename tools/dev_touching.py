...
def _through_a_tool(path):
    """`{test: reason}` from `record_readers` on the tree; `{}` for a module."""
    if path.endswith(".py"):
        return {}
    corpus = {c: _candidate_text(c) for c in test_files()}
    found = record_readers(path, _tool_sources(),
                           {c: t for c, t in corpus.items() if t is not None})
    return {c: f"{path} via {', '.join(reads)}" for c, reads in found.items()}


def _choose(chosen, why, reasons):
    for candidate, reason in reasons.items():
        chosen[candidate] = True
        why.setdefault(candidate, []).append(reason)
...
def select(changed, census=True):
    ...
    for path in changed:
        ...
        # `UX-942`: a record read through a tool's constant names neither.
        _choose(chosen, why, _through_a_tool(path))
