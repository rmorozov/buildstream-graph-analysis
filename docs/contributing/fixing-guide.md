...
is ~60 KB because each one carries the incident that produced it, and
...
3. **While you work, run the tests that touch what you changed** (`UX-336`): `make test-touching` maps the working diff to the test files that name it - run `python3 tools/dev_touching.py --spread` for the spread over every module the map names, not the seconds one machine spent on one of them (`UX-632`); the figure is never committed (`UX-996`) and moves whenever the set of (test file, module named) pairs changes - a new test file, a new import, a renamed module, a deleted file - not only the first of those (`UX-756`: round 103 moved it with two imports and no new file, and a committed row reddened stale). Wider than one module, run the tier (`UX-238`). Every target runs `-n auto`. **The suite's wall clock is a property of the machine, not of the suite** (`UX-551`), so budget a round against the spread and not a figure:
...
Round 80's 8m52s is **not reproducible on the tree that produced it**: the same commit reads 3m32s on a quiet machine. The suite grew by 621 tests between rounds 74 and 81 and got *faster* in wall clock. So a single dated sample dates the afternoon, not the suite — measure it yourself when the number matters, and record the load average beside it. The small tier is 20s:
   ...
   |---|---|---|
   | `make test-touching` | `dev_touching.py --spread` | the test files that name what your diff touched |
...
bga/floors/            capacity.py, cold.py, cpu.py, observed.py, serialization.py - the certified floors
...
bga/buildclass.py      the declared build type and variant; the
...
tools/sysroot_manifest.py    what the examples' sysroot is - one row per
                         ...
                         staged copy reports (UX-914)
tools/toolchain_params.py    where that sysroot's own driver reaches - one
                         row per file class, asked rather than assumed,
                         and the PATH shim that carries the flags (UX-930)
...
2. The index counts, printed rather than committed (`UX-501`, `UX-996`):
   `dev_close_task.py --counts` —
...
4. The touch-map spread, printed rather than committed (`UX-756`,
   `UX-996`): `dev_touching.py --spread` —
