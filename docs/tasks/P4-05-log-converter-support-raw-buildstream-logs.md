...
**Two real bugs found while implementing, verified against a real, installed BuildStream 2.7.0** (not assumed from the log line shape alone - see `docs/ingestion-pipeline.md`'s "Empirically confirmed facts about real BuildStream logs" for the full list):
...
- `tools/chrome_trace_to_bga_trace.py` (the Chrome-Trace-to-trace/v9 conversion) was built as a separate, new, general-purpose tool - not folded into this one, and not the same as the synthetic fixture's own `adapter.py`. See `docs/ingestion-pipeline.md`'s "Why a second, separate trace/v9 adapter" section.
