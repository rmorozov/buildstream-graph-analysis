...
Read only: `sed -n '869,904p' docs/spec/specification.md` (Part 14 — Structural Floors, 14.2 Unweighted Depth). `unweighted_depth` must be the **longest** path in hops from a root, independent of duration — this is exactly what `bga/graph/edg.py::compute_unweighted_depth` correctly computes (verified below).
