...
#: `UX-688`: the areas are the module tree the fixing guide's §6
#: already maintains, read from it rather than typed here - a second
#: list of directories is a second list to drift.
AREA_GUIDE = REPO / "docs/contributing/fixing-guide.md"

#: `UX-501`'s bucket, one field over. 269 of 714 rows touch no code at
#: all (they are documents or process) and 145 more name two areas at
#: once; both go here rather than being distributed by guesswork, and
#: the bucket shrinks as rounds classify.
AREA_UNKNOWN = "unassigned"
...
def header_area(text):
    """The `**Area:**` a task file declares, from its first 8 lines."""
    found = _AREA_HEADER.search("\n".join(text.splitlines()[:8]))
    return found.group(1) if found else None


def file_areas():
    """`{uid: area}` for every task file that declares one.

    The task files, the population `file_topics` uses - not
    `backlog_files()`, which is the two index documents.
    """
    found = {}
    for path in sorted(SCENARIOS.glob("UX-*.md")):
...
def _on_the_real_index():
    """`--scenarios` points the index checks at a sandbox: `UX-935`'s
    merge-in-progress refusal only applies to the repository's own
    git index, not a test's temp directory."""
...
def _backlog_counts():
    """`{directory: file count a commit from here would carry}` for
    `docs/backlog/scenarios/` and `docs/backlog/tasks/`.

    `UX-996` stopped this being written into `architecture.md`'s opening
    sentence - `dev_tier_drift.py`'s population scaling (`UX-716`) still
    reads it live, which is not a committed figure.
    ...
    `.claude/worktrees/agent-1/`, measured. `--others --exclude-standard`
...
def move(uid: str, note: str) -> int:
    ...
    # `UX-501`/`UX-996`: the aggregates are never committed. Measured on
    # two branches each closing one item: writing them made the topic
    # table conflict - the two topics' rows are adjacent, so git reads
...
def move_batch(pairs: list) -> int:
    ...
    print(f"counts derived, never committed: {sentence}")
    return 0
