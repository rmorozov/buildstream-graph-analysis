...
Added regression tests to `tests/unit/test_pipeline_overhead.py` (4 new tests, real captured log excerpts for both cases). See `docs/tasks/P4-15-stack-consolidation-heuristic.md` for what the now-correctly-routed per-element checkout data is used for (`tools/bst_checkout_cost.py`).
