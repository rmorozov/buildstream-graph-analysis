...
| **a table-less kind whose sandbox ninja already holds a client** | the probe finds a ninja that already speaks the protocol; injecting `MAKEFLAGS` again would double the pool | the shim's `ninja_client` branch | `ninja_client` | guarded | `tests/unit/test_bwrap_shim.py` |
