...
context at each rebuild, 73 % of its tokens when measured; a result over a screen goes to the scratchpad (`UX-711`). `measure`, `falsify`, `verify`
(which calls `self-review` last) run inside a track; `derive` before moving viewer code; `walk`, `design-review` and `review` audit the page; every run is a row in `docs/audits/agent-runs.md`.
