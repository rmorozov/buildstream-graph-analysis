...
| `BST_TRACE_JOBSERVER_AUTH_MAP` | `bga capture run --jobserver-auth-override`'s own map (`style:glob[,glob];...`), resolved in `bga/cli.py` and carried unchanged through `tools/bst_native_build_tracer.py`; the shim's `resolve_auth_override` matches it against the element name and forces the style, ahead of the auto/`compiler_safe` path (`UX-879`) | `tools/native_trace/bwrap_shim.py` |
...
- `bga capture run --jobserver-auth-override 'fd:<glob>[,<glob>] fifo:<glob> off:<glob>'` (`UX-879`; repeatable, or one value with space-separated groups) — a per-element override of the style above, resolved by the shim against the element name (`fnmatch`) and taking precedence over the auto/`compiler_safe` decision: `fd` forces the raw, pre-`compiler_safe` auth (no fifo rewrite, no scrub); `fifo` forces the `fifo:` path; `off` scrubs (no `--jobserver-auth`, no wrapper mount, `JOBS` left alone so the recipe's own `-jN` stands). First matching glob wins. Translated to `BST_TRACE_JOBSERVER_AUTH_MAP` before the tracer ever sees it, same channel as `--jobserver`'s own `auto`/`N`/`off` resolution. **Caveat**: forcing `fd` on an element that does LTO (`gcc -flto`'s lto-wrapper reads `MAKEFLAGS` directly, an unwrapped deep grandchild) will hit the GCC-13 ICE `compiler_safe` exists to avoid, until the compiler shim (round 126) lands — use `fd` only for a non-LTO element on a make ≤4.2.1, `off` for an LTO one meanwhile.
