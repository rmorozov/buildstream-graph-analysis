...
def _report(*peaks_mb):
    """A Plane 2 report, in Plane 2's own units.

    `UX-341`: the record still reports `ru_maxrss` in KiB - it is an
    input, not one of this tool's documents - and the conversion to the
    bytes the payload publishes happens in `compute_memory_envelope`.
...
def test_the_envelope_sums_the_n_largest_peaks():
    ...
    envelope = compute_memory_envelope(_report(400, 300, 200, 100), 2, 8589934592)
    at_two = envelope['at_observed_builders']
    assert at_two['builders'] == 2
    # the two largest, not the two first
    assert at_two['envelope_bytes'] == 700 * 1024 * 1024
...
def test_the_first_builders_count_that_does_not_fit_is_named():
    ...
    envelope = compute_memory_envelope(_report(4000, 4000, 4000, 4000), 2, 9437184000)
...
def test_no_measured_peaks_means_no_envelope():
    assert compute_memory_envelope({'peak_memory': {'per_element': {}}}, 2, 8589934592) == {}
...
def test_the_projection_stops_at_the_measured_population():
    ...
    envelope = compute_memory_envelope(_report(400, 300), 4, 8589934592)
...
def test_no_reserve_is_invented_and_the_payload_says_so():
    ...
    envelope = compute_memory_envelope(_report(4096, 4000), 2, 8589934592)
...
def test_the_sentence_is_the_one_the_readme_used_to_ask_for_in_prose():
    envelope = compute_memory_envelope(_report(2000, 1900, 1800, 1700), 4, 16777216000)
...
def test_a_build_where_memory_does_not_bind_says_that_instead():
    ...
    ceiling."""
    envelope = compute_memory_envelope(_report(160, 155, 150, 145, 140, 135, 130), 4, 16777216000)
    line = _memory_envelope_findings(_result(envelope))[0]
...
def test_more_builders_is_refused_on_memory_grounds():
    ...
    envelope = compute_memory_envelope(_report(3000, 3000, 3000, 3000), 2, 8388608000)
...
def test_a_ceiling_further_away_than_one_more_builder_is_not_a_refusal():
    ...
    that would fit."""
    envelope = compute_memory_envelope(_report(1000, 1000, 1000, 1000, 1000), 2, 4718592000)
