...
1. A new `docs/guides/optimization-walkthrough.md`: a narrative, worked example starting from a real example project, showing:
...
1. `docs/guides/optimization-walkthrough.md` exists, every command in it runs against a real (not fixture-only) example project and produces the exact output shown.
...
Full transcript with every real command and its actual output: [`docs/guides/optimization-walkthrough.md`](../../guides/optimization-walkthrough.md). Linked from `README.md`'s Documentation section. `make lint`/`pytest` both green (458 passed; 7 pre-existing failures unrelated to this change, confirmed via `git stash` against `main`). PR: https://github.com/rmorozov/buildstream-graph-analysis/pull/43

Two real bugs were found along the way (both deferred to backlog per this work session's scope, not fixed here - see `docs/backlog/scenarios/UX-0006-raw-log-timestamp-corruption.md` and `docs/backlog/scenarios/UX-0007-run-identity-collides-across-sibling-projects.md`): `--format raw`'s timestamp reconstruction corrupts cross-task ordering on real saved logs (worked around here via a new `tools/bst_run_wrapped.py` live-capture tool instead), and `run_identity.manifest_hash` collides for two different projects under the same git commit.
